...
# ------------------------------------------------------------------
# FIX 1: Sacred compatibility with modern Python
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# FIX 2: pprint compatibility (older sacred expects this)
# ------------------------------------------------------------------
...
def config():
    ...
    train_on = 'MAPS'   # ✅ FIXED (was MAESTRO)
    ...
    if torch.cuda.is_available() and torch.cuda.get_device_properties(torch.cuda.current_device()).total_memory < 10e9:
        sequence_length //= 2
        print(f'Reducing sequence_length to {sequence_length} to save memory')
...
# ------------------------------------------------------------------
@ex.automain
def train(logdir, device, iterations, resume_iteration, checkpoint_interval, train_on,
          batch_size, sequence_length, model_complexity,
          learning_rate, learning_rate_decay_steps, learning_rate_decay_rate,
          leave_one_out, clip_gradient_norm,
    ...
    # ------------------------------------------------------------------
    dataset = MAPS(
        groups=['AkPnBcht', 'AkPnBsdf', 'AkPnCGdD', 'AkPnStgb',
